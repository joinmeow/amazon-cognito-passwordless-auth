...
const mockTimers = () => {
  ...
  const intervals = new Map<number, { callback: () => void; interval: number; lastRun: number }>();
  let nextTimerId = 1;
...
describe("Same-Tab Refresh Behavior", () => {
  ...
  beforeEach(() => {
    ...
    configure({
      ...
      storage: mockStorage,
      debug: (...args: any[]) => {
        debugLogs.push(`[${Date.now()}] ${args.join(" ")}`);
      },
    });
  });
  ...
  describe("Schedule Refresh Timer Behavior", () => {
    test("should set up timer correctly for tokens expiring in future", async () => {
      ...
      const schedLogs = debugLogs.filter(log => log.includes("Scheduling token refresh"));
    ...
    test("should trigger immediate refresh for expired tokens", async () => {
      ...
      const expiredLog = debugLogs.find(log => 
  ...
  describe("Watchdog Timer Behavior", () => {
    ...
    test("should detect device sleep and adjust refresh timing", async () => {
      ...
      const sleepLog = debugLogs.find(log => 
  ...
  describe("Error Scenarios and Edge Cases", () => {
    test("should handle rapid successive refresh attempts", async () => {
      ...
      const dedupLog = debugLogs.find(log => 
        log.includes("already refreshing") || 
        log.includes("Another refresh")
      );
    ...
    test("should handle missing expireAt gracefully", async () => {
      ...
      await storeTokens(tokens as any);
